...
then
	...
	#
       ...
       then
               ...
               # retcode=2 request reboot. note that 3=2+1.
		...
		then
		    # Surprise! Re-directing from a HERE document (as in
                    ...
                    log_status_msg ""
                    log_status_msg "fsck failed (exit code $RET). Please repair manually and reboot."
                    log_status_msg "Please note that the root file system is currently mounted read-only."
                    log_status_msg "To remount it read-write:"
                    log_status_msg ""
		    ...
		    then
			log_status_msg ""
			log_status_msg "fsck corrected errors on the root partition, but requested that"
			log_status_msg "the system be rebooted (exit code $RET)."
			log_status_msg ""
			log_status_msg "Automatic reboot in 5 seconds."
			log_status_msg ""
			sleep 5
			$DAEMON7 -f
		    else
