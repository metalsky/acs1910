...
void showUsage()
{
  ...
  OSA_printf(" NTSC / PAL   : Mode for Flicker Detect\n");
...
int main(int argc, char **argv)
{
  ...
  gAVSERVER_UI_config.codec 			= ALG_VID_CODEC_NONE;
  gAVSERVER_UI_config.aewbEnable 		= FALSE;
  gAVSERVER_UI_config.afEnable 			= TRUE;
  gAVSERVER_UI_config.aewbType 			= AEWB_OFF;
  ...
  for(i=1; i< argc; i++) {
    ...
    if(strcmp(argv[i], "1080P")==0)
      gAVSERVER_UI_config.mode = AVSERVER_UI_CAPTURE_MODE_1080P;
    else
    if(strcmp(argv[i], "1080P_D1")==0)
      gAVSERVER_UI_config.mode = AVSERVER_UI_CAPTURE_MODE_1080P_D1;
    else
    ...
    if(strcmp(argv[i], "720P_VGA_30")==0)
      gAVSERVER_UI_config.mode = AVSERVER_UI_CAPTURE_MODE_720P_VGA_30;
    else
