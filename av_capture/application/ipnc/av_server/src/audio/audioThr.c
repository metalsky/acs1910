...
int AUDIOPLAY_audioTskRun(Bool *pDone)
{
	int rc = -1;
	DRV_AudioConfig Audioplayconfig;
	...
	switch(rbuf.msg)
	{
	    case AUD_STREAM_NEWFRAME:
			   ...
			   (gAUDIOPLAY_ctrl.audPlayMode == AUD_PLAY_MODE_PLAY))
			{
				...
				// Open the device
				Audioplayconfig.samplingRate		= rbuf.sampleRate;
				Audioplayconfig.deviceId			= 0;
				Audioplayconfig.numChannels 		= 1;
				Audioplayconfig.buff_Of_Samples 	= 640;
				Audioplayconfig.bufferSize			= 1280;
			...
			{
				rc = DRV_audioWrite(&gAUDIOPLAY_ctrl.audioHndl, sharedAudioData->buf[0], sharedAudioData->pktSize/2);
			}
...
int AUDIOPLAY_audioTskMain(struct OSA_TskHndl *pTsk, OSA_MsgHndl *pMsg, Uint32 curState )
{
  ...
  while(!done) {
    ...
    status = AUDIOPLAY_audioTskRun(&done);
