...
char* RTSPServer::rtspURLPrefix(int clientSocket) const {
  ...
  if (portNumHostOrder == 554 /* the default port number */) {
    sprintf(urlBuffer, "rtsp://%s/PSIA/Streaming/channels/", our_inet_ntoa(ourAddress.sin_addr));  //PSIA/Streaming/channels/
  }
/*  else if (portNumHostOrder == 8554) {
    sprintf(urlBuffer, "rtsp://%s/PSIA/Streaming/channels/", our_inet_ntoa(ourAddress.sin_addr));
  }
  else if (portNumHostOrder == 8555) {
    sprintf(urlBuffer, "rtsp://%s/PSIA/Streaming/channels/", our_inet_ntoa(ourAddress.sin_addr));
  }
  else if (portNumHostOrder == 8556) {
    sprintf(urlBuffer, "rtsp://%s/PSIA/Streaming/channels/", our_inet_ntoa(ourAddress.sin_addr));
  }
  else if (portNumHostOrder == 8557) {
    sprintf(urlBuffer, "rtsp://%s/PSIA/Streaming/channels/", our_inet_ntoa(ourAddress.sin_addr));
  }
 */ else {
    sprintf(urlBuffer, "rtsp://%s:%hu/PSIA/Streaming/channels/",
	    our_inet_ntoa(ourAddress.sin_addr), portNumHostOrder);  //PSIA/Streaming/channels/
  }
