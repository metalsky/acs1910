	...
	sampleRate = params_rate(params);
 	...
 	if (cpu_is_davinci_dm365()) {
		if(sampleRate != gSampleRate)
		{
			...
			prediv = (davinci_readl(DAVINCI_PLL_CNTRL1_BASE + PREDIV) &	0x1f) + 1;
			postdiv = (davinci_readl(DAVINCI_PLL_CNTRL1_BASE + POSTDIV) & 0x1f) + 1;
			pll_rate = ((fixedrate / prediv) * (2 * pll1_mult)) / postdiv;
			pll_div4 = (davinci_readl(DAVINCI_PLL_CNTRL1_BASE + PLLDIV4) & 0x1f) + 1; /* VOICE */
