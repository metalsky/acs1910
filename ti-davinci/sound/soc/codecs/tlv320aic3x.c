 ...
 */
#if !defined(CONFIG_SND_SOC_TLV320AIC3101)
...
static const u8 *aic3x_cache = aic3x_reg;
static short pg0_end = ARRAY_SIZE(aic3x_reg);
#else
static const u8 aic3101_reg[] = {
	0x00, 0x00, 0x00, 0x10,	/* 0 */
	0x04, 0x00, 0x00, 0x00,	/* 4 */
	0x00, 0x00, 0x00, 0x01,	/* 8 */
	0x00, 0x00, 0x00, 0x80,	/* 12 */
	...
	0x00, 0x00, 0x00, 0x80,	/* 40 */
	0x80, 0x00, 0x00, 0x00,	/* 44 */
	0x00, 0x00, 0x00, 0x06,	/* 48 */
	0x00, 0x00, 0x00, 0x00,	/* 52 */
	0x00, 0x00, 0x06, 0x00,	/* 56 */
	0x00, 0x00, 0x00, 0x00,	/* 60 */
	0x00, 0x06, 0x00, 0x00,	/* 64 */
	0x00, 0x00, 0x00, 0x00,	/* 68 */
	0x06, 0x00, 0x00, 0x00,	/* 72 */
	0x00, 0x00, 0x00, 0x00,	/* 76 */
	0x00, 0x00, 0x00, 0x00,	/* 80 */
	0x00, 0x00, 0x02, 0x00,	/* 84 */
	0x00, 0x00, 0x00, 0x00,	/* 88 */
	0x00, 0x02, 0x00, 0x00,	/* 92 */
	0x00, 0x00, 0x00, 0x00,	/* 96 */
	0x00, 0x00, 0x02, 0x00,	/* 100 */
	0x00, 0x00, 0x00, 0x00,	/* 104 */
	0x00, 0x00,	/* 108 */
};
static const u8 *aic3x_cache = aic3101_reg;
static short pg0_end = ARRAY_SIZE(aic3101_reg);
#endif
/*
...
static int snd_soc_aic3x_info_recsrc(snd_kcontrol_t *kcontrol,
					snd_ctl_elem_info_t *uinfo) {
	char *text[2] =	{ "Mic", "Line"	};
	...
	strcpy(uinfo->value.enumerated.name,
		text[uinfo->value.enumerated.item]);

	return 0;
}
static int snd_soc_aic3x_get_recsrc(snd_kcontrol_t *kcontrol,
					snd_ctl_elem_value_t *ucontrol) {
	ucontrol->value.enumerated.item[0] = 0;
	return 0;
}

static int snd_soc_aic3x_put_recsrc(snd_kcontrol_t *kcontrol,
					snd_ctl_elem_value_t *ucontrol) {
	struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol);
	int changed = 1;
	...
	if (!ucontrol->value.integer.value[0]) {
		...
		aic3x_write(codec, LINE1L_2_LADC_CTRL, 0x7C);
		aic3x_write(codec, LINE1R_2_RADC_CTRL, 0x7C);
	} else {
		aic3x_write(codec, MIC3LR_2_LADC_CTRL, 0xff);
		aic3x_write(codec, MIC3LR_2_RADC_CTRL, 0xff);
		aic3x_write(codec, LINE1L_2_LADC_CTRL, 0x4);
		aic3x_write(codec, LINE1R_2_RADC_CTRL, 0x4);
	}
	return changed;
}

#define AIC3X_RECSRC(xname) \
{ \
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER, \
	.name = xname, \
	.info = snd_soc_aic3x_info_recsrc, \
	.get = snd_soc_aic3x_get_recsrc, \
	.put = snd_soc_aic3x_put_recsrc, \
}

static const struct snd_kcontrol_new aic3x_snd_controls[] = {
	/* Output */
	SOC_DOUBLE_R("Master Playback Volume", LDAC_VOL, RDAC_VOL, 0, 0x7f, 1),
...
static int aic3x_init(struct snd_soc_device *socdev)
{
	...
	codec->dai = &aic3x_dai;
	codec->num_dai = 1;
	codec->reg_cache_size = pg0_end;
	codec->reg_cache = kmemdup(aic3x_cache, pg0_end, GFP_KERNEL);
	...
	/* Enable AGC */
	aic3x_write(codec, LAGC_CTRL_A, 0x80);
	aic3x_write(codec, LAGC_CTRL_B, 0x00);
	aic3x_write(codec, LAGC_CTRL_C, 0x00);
	aic3x_write(codec, RAGC_CTRL_A, 0x80);
	aic3x_write(codec, RAGC_CTRL_B, 0x00);
	aic3x_write(codec, RAGC_CTRL_C, 0x00);
	...
	aic3x_write(codec, LINE1L_2_LADC_CTRL, 0x04);
	aic3x_write(codec, LINE1R_2_RADC_CTRL, 0x04);
