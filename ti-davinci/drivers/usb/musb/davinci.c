...
static inline void phy_off(void)
{
	...
	else if (cpu_is_davinci_dm6467() || cpu_is_davinci_dm365())
...
int __init musb_platform_init(struct musb *musb)
{
...
#ifdef CONFIG_USB_MUSB_OTG
	{
		...
		if (cpu_is_davinci_dm644x())
			...
			davinci_cfg_reg(DM357_GPIO3V, PINMUX_RESV);
	}
#endif
#endif
	if (machine_is_davinci_dm365_evm()) {
		davinci_cfg_reg(DM365_GPIO33, PINMUX_RESV);
...
int musb_platform_exit(struct musb *musb)
{
	...
	if (cpu_is_davinci_dm644x())
		...
		davinci_cfg_reg(DM357_GPIO3V, PINMUX_FREE);
#endif
	if (machine_is_davinci_dm365_evm())
		davinci_cfg_reg(DM365_GPIO33, PINMUX_FREE);
