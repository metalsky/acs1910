...
static irqreturn_t davinci_disp_isr(int irq, void *arg, struct pt_regs *regs)
{
	...
	if (cpu_is_davinci_dm365()) {
		if (!
		    (davinci_readl(DM365_ISP5_REG_BASE + ISP5_INTSTAT) &
		     ISP5_INTSTAT_VENCINT)) {
			return IRQ_NONE;
		}
		davinci_writel(ISP5_INTSTAT_VENCINT,
			       DM365_ISP5_REG_BASE + ISP5_INTSTAT);
	}
				   ...
				   enum davinci_v_exp_ratio v_exp)
{
	...
	if (h_exp == H_EXP_3_OVER_2 &&
			(cpu_is_davinci_dm644x() || cpu_is_davinci_dm357()))
		return -1;
	...
	case WIN_VID0:
		...
		if (cpu_is_davinci_dm365()) {
			if (lconfig->pixfmt == PIXFMT_NV12) {
				 ...
				 */
				if (lconfig->xsize % 32 && lconfig->line_length == width) {
					osd_write(width,
						  OSD_VIDWIN1XL);
					osd_write(width,
						  OSD_VIDWIN0XL);
...
int davinci_disp_init(void)
{
	...
	if (cpu_is_davinci_dm644x() || cpu_is_davinci_dm357()) {
		...
		osd->osdregs = DM365_OSD_REG_BASE;
		osd->vencregs = DM365_VENC_REG_BASE;
	} else
		return -1;

	osd->irq = VPSS_VENCINT;
